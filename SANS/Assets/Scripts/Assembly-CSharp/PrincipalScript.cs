...
public class PrincipalScript : MonoBehaviour
{
	...
	private void TargetBully()
	{
		if (!this.bullySeen)
		{
			...
			this.bullySeen = true;
			sm.Add3DSubtitle("No bullying in the halls!", audNoBullying.length, Color.white, base.transform);
		}
	}
	...
	private void CorrectPlayer()
	{
		...
		if (this.playerScript.guiltType == "faculty")
		{
			...
			sm.Add3DSubtitle("No entering school faculty only rooms in the halls!", audNoFaculty.length, Color.white, base.transform);
		}
		else if (this.playerScript.guiltType == "running")
		{
			this.audioQueue.QueueAudio(this.audNoRunning);
			sm.Add3DSubtitle("No running in the halls!", audNoRunning.length, Color.white, base.transform);
		}
		else if (this.playerScript.guiltType == "drink")
		{
			this.audioQueue.QueueAudio(this.audNoDrinking);
			sm.Add3DSubtitle("No drinking drinks in the halls!", audNoDrinking.length, Color.white, base.transform);
		}
		else if (this.playerScript.guiltType == "escape")
		{
			this.audioQueue.QueueAudio(this.audNoEscaping);
			sm.Add3DSubtitle("No escaping detention in the halls!", audNoEscaping.length, Color.white, base.transform);
		}
	}
